...
class DataDB(object):
    ...
    def unit_config(self, unitid, branchid):
        get_module_id1_sql = """SELECT config from aidb.unitconfig_tb WHERE unitid =%s AND branch =%s;""" % (
            str(unitid), str(branchid))
        return self.exec_sql(get_module_id1_sql)
    ...
    def all_func(self):
        get_module_id1_sql = """SELECT func_name from aidb.func_tb;"""
        # print(get_module_id1_sql)
        return self.exec_sql(get_module_id1_sql)
