...
# from ai_sets.model import Metadata, Interpreter
...
class Project(object):
    def __init__(self, config=None, modelpath=None, component_builder=None, project=None):
        # 1. 但项目的模型加载
        self._config = config
        ...
        if project:
            self._path = os.path.join(self._path, project)
        # defaultint = utils.find_max(os.path.join(self._path, ""))
    ...
    def _list_models_in_dir(path):
        if not path or not os.path.isdir(path):
            ...
            return [os.path.relpath(model, path) for model in glob.glob(os.path.join(path, '*')) if
                    os.path.isdir(model)]
