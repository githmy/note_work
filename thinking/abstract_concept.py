...
class ConceptNet:
    ...
    def load_concept_edges(self, ):
        edges = []
        for line in open(self.hiearchy_file, encoding="utf-8"):
    ...
    def build_all_concepts(self):
        ...
        for line in open(self.concept_file, encoding="utf-8"):
            line = line.strip().split('\t')
            wd = line[0]
    ...
    def gets_hiearchy(self, sourcewords):
        res_hiearchy = []
        for sourceword in sourcewords:
            tmp_hiearchy = []
            paths = self.all_dict.get(sourceword, '')
            if paths:
                for path in paths:
                    tmp_hiearchy.append('|'.join(path))
            res_hiearchy.append(tmp_hiearchy)
        return res_hiearchy
