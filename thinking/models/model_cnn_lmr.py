        ...
        with tf.Graph().as_default():
            ...
            with tf.name_scope("output"):
                ...
                W_m = tf.get_variable(
                    ...
                    initializer=tf.contrib.layers.xavier_initializer())
                b_m = tf.Variable(tf.constant(0.1, shape=[self.label_length_m]), name="b_m")
                l2_loss_m += tf.nn.l2_loss(W_m)
                l2_loss_m += tf.nn.l2_loss(b_m)
                self.scores_m = tf.nn.xw_plus_b(self.h_drop, W_m, b_m, name="scores_m")
                self.predictions_m = tf.nn.sigmoid(self.scores_m)
                tf.summary.histogram('predictions_m', self.predictions_m)
                # self.predictions_m = tf.argmax(self.scores_m, 1, name="predictions_m")
                ...
                self.predictions_r = tf.nn.sigmoid(self.scores_r)
                tf.summary.histogram('predictions_m', self.predictions_r)
                # self.predictions_r = tf.argmax(self.scores_r, 1, name="predictions_r")
            ...
            with tf.name_scope("accuracy"):
                ...
                tf_dim_in_m = tf.reduce_sum(tf.abs(tf.round(self.predictions_m) - self.input_y_m), 1)
                ...
                tf_dim_in_r = tf.reduce_sum(tf.abs(tf.round(self.predictions_r) - self.input_y_r), 1)
