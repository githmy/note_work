...
from utils.log_tool import basic_path, project_path, model_path, log_path, data_path, conf_path


# 构建模型
class TextCNN(object):
    def __init__(self, model_type, model_name, model_json):
    ...
    def get_shape(self):
        dict_file = os.path.join(self.data_dir, "review_obj.csv")
        ...
        self.label_length_l = len(["1", "2", "3", "4", "5", "6"])
    ...
    def build(self):
        if 1:
            ...
            with tf.name_scope('input'):
                ...
                self.weigh_l = tf.placeholder(tf.float32, [self.label_length_l], name='weigh_l')
                self.weigh_r = tf.placeholder(tf.float32, [self.label_length_r], name='weigh_r')
                self.weigh_m = tf.placeholder(tf.float32, [self.label_length_m], name='weigh_m')
            ...
            with tf.name_scope("output"):
                ...
                self.predictions_r = tf.round(tf.nn.sigmoid(self.scores_r) * self.weigh_r)
                tf.summary.histogram('scores_r', self.scores_r)
                tf.summary.histogram('predictions_r', self.predictions_r)
                ...
                self.predictions_m = tf.round(
                    tf.nn.sigmoid(tf.multiply(self.predictions_r, tf.nn.sigmoid(self.scores_m * self.weigh_m))))
                tf.summary.histogram('scores_m', self.scores_m)
                tf.summary.histogram('predictions_m', self.predictions_m)

                origin_and_l = tf.concat([self.h_drop, self.scores_r * 0.00001], 1)
                tf.summary.histogram('origin_and_l', origin_and_l)
                # 3层全连接
                with tf.name_scope("full_connect"):
                    tmpint = num_filters_total + self.label_length_r
                    tmpint2 = int(tmpint / 2)
                    W_l1 = tf.get_variable(
                        "W_l1",
                        shape=[tmpint, tmpint2],
                        initializer=tf.contrib.layers.xavier_initializer())
                    b_l1 = tf.Variable(tf.constant(0.1, shape=[tmpint2]), name="b_l1")
                    l2_loss_l += tf.nn.l2_loss(W_l1)
                    l2_loss_l += tf.nn.l2_loss(b_l1)
                    mid_l1 = tf.nn.xw_plus_b(tf.nn.leaky_relu(origin_and_l), W_l1, b_l1)
                    tmpint4 = int(tmpint2 / 2)
                    W_l2 = tf.get_variable(
                        "W_l2",
                        shape=[tmpint2, tmpint4],
                        initializer=tf.contrib.layers.xavier_initializer())
                    b_l2 = tf.Variable(tf.constant(0.1, shape=[tmpint4]), name="b_l2")
                    l2_loss_l += tf.nn.l2_loss(W_l2)
                    l2_loss_l += tf.nn.l2_loss(b_l2)
                    mid_l2 = tf.nn.xw_plus_b(tf.nn.leaky_relu(mid_l1), W_l2, b_l2)
                    tmpint8 = int(tmpint4 / 2)
                    W_l4 = tf.get_variable(
                        "W_l4",
                        shape=[tmpint4, tmpint8],
                        initializer=tf.contrib.layers.xavier_initializer())
                    b_l4 = tf.Variable(tf.constant(0.1, shape=[tmpint8]), name="b_l4")
                    l2_loss_l += tf.nn.l2_loss(W_l4)
                    l2_loss_l += tf.nn.l2_loss(b_l4)
                    mid_l3 = tf.nn.xw_plus_b(tf.nn.leaky_relu(mid_l2), W_l4, b_l4)
                    tmpint16 = int(tmpint8 / 2)
                    W_l8 = tf.get_variable(
                        "W_l8",
                        shape=[tmpint8, tmpint16],
                        initializer=tf.contrib.layers.xavier_initializer())
                    b_l8 = tf.Variable(tf.constant(0.1, shape=[tmpint16]), name="b_l8")
                    l2_loss_l += tf.nn.l2_loss(W_l8)
                    l2_loss_l += tf.nn.l2_loss(b_l8)
                    mid_l4 = tf.nn.xw_plus_b(tf.nn.leaky_relu(mid_l3), W_l8, b_l8)
                    tmpint32 = int(tmpint16 / 2)
                    W_l16 = tf.get_variable(
                        "W_l16",
                        shape=[tmpint16, tmpint32],
                        initializer=tf.contrib.layers.xavier_initializer())
                    b_l16 = tf.Variable(tf.constant(0.1, shape=[tmpint32]), name="b_l16")
                    l2_loss_l += tf.nn.l2_loss(W_l16)
                    l2_loss_l += tf.nn.l2_loss(b_l16)
                    mid_l5 = tf.nn.xw_plus_b(tf.nn.leaky_relu(mid_l4), W_l16, b_l16)
                    tmpint64 = int(tmpint32 / 2)
                    W_l32 = tf.get_variable(
                        "W_l32",
                        shape=[tmpint32, tmpint64],
                        initializer=tf.contrib.layers.xavier_initializer())
                    b_l32 = tf.Variable(tf.constant(0.1, shape=[tmpint64]), name="b_l32")
                    l2_loss_l += tf.nn.l2_loss(W_l32)
                    l2_loss_l += tf.nn.l2_loss(b_l32)
                    mid_l6 = tf.nn.xw_plus_b(tf.nn.leaky_relu(mid_l5), W_l32, b_l32)
                    tmpint128 = int(tmpint4 / 2)
                    W_l64 = tf.get_variable(
                        "W_l64",
                        shape=[tmpint64, self.label_length_l],
                        initializer=tf.contrib.layers.xavier_initializer())
                    b_l64 = tf.Variable(tf.constant(0.1, shape=[self.label_length_l]), name="b_l64")
                    l2_loss_l += tf.nn.l2_loss(W_l64)
                    l2_loss_l += tf.nn.l2_loss(b_l64)
                self.scores_l = tf.nn.xw_plus_b(tf.nn.leaky_relu(mid_l6), W_l64, b_l64, name="scores_l")
                self.predictions_l = tf.argmax(self.scores_l * self.weigh_l, 1, name="predictions_l")
    ...
    def fit(self, x_train, x_dev, y_train_m, y_dev_m, y_train_r, y_dev_r, y_train_l, y_dev_l):
        ...
        oweigh_r = np.ones(self.label_length_r)
        oweigh_m = np.ones(self.label_length_m)
        oweigh_l = np.ones(self.label_length_l)
        best_loss = 1e9
        best_counter = 0
        # 9. 辅助参数
            ...
            with tf.Session(config=session_conf) as sess:
                ...
                for batch in batches:
                    ...
                    feed_dict = {
                        ...
                        self.weigh_l: oweigh_l,
                        self.weigh_m: oweigh_m,
                        self.weigh_r: oweigh_r,
        ...
        if 1:
            ...
            with tf.Session(config=session_conf) as sess:
                ...
                for batch in test_batches:
                    feed_dict = {
                        self.input_x: batch,
                        self.weigh_l: self.user_weigh_l,
                        self.weigh_m: self.user_weigh_m,
                        self.weigh_r: self.user_weigh_r,
                        self.dropout_keep_prob: 1.0
                    }
                    p_r, p_m, p_l = sess.run([self.predictions_r, self.predictions_m, self.predictions_l], feed_dict)
                    all_predictions_r.extend(list(p_r))
                    all_predictions_m.extend(list(p_m))
                    all_predictions_l.extend(list(p_l))
                print("prediction finished!")
                return all_predictions_r, all_predictions_m, all_predictions_l
    ...
    def _get_standard_data(self):
        ...
        if os.getenv('prtest') is None:
            ...
            # 1.1 数据读取
            # train_file = os.path.join(project_path, "question_obj.csv")
            # test_file = os.path.join(project_path, "predict_obj.csv")
            train_file = os.path.join(self.data_dir, "train_compare_origin.csv")
            test_file = os.path.join(self.data_dir, "train_compare_label.csv")
    ...
    def get_weigh(self):
        # 获取权重
        self.user_weigh_r = np.zeros(self.label_length_r)
        self.user_weigh_m = np.zeros(self.label_length_m)
        self.user_weigh_l = np.zeros(self.label_length_l)
        if os.environ['prtest'] is None:
            pass
        else:
            review_json = json.load(open(os.path.join(self.conf_dir, "review_weigh.json"), encoding="utf8"))
            main_json = json.load(open(os.path.join(self.conf_dir, "main_weigh.json"), encoding="utf8"))
            level_json = json.load(open(os.path.join(self.conf_dir, "level_weigh.json"), encoding="utf8"))
            self.user_weigh_r = np.array([review_json[self.dict_points[i1]] for i1 in self.label_list])
            self.user_weigh_m = np.array([main_json[self.dict_points[i1]] for i1 in self.label_list])
            self.user_weigh_l = np.array([level_json[str(i1 + 1)] for i1 in range(self.label_length_l)])
