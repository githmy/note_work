    ...
    def build(self):
        with tf.Graph().as_default():
            ...
            with tf.name_scope("output"):
                ...
                # origin_and_l = tf.Variable(tf.concat([self.h_drop, self.scores_r], 1), trainable=False)
                origin_and_l = tf.concat([self.h_drop, self.scores_r], 1)
                # freeze_h_drop = tf.stop_gradient(origin_and_l)
                freeze_origin_and_l = tf.stop_gradient(origin_and_l)
                tf.summary.histogram('freeze_origin_and_l', freeze_origin_and_l)
                W_l = tf.get_variable(
                    "W_l",
                ...
                self.scores_l = tf.nn.xw_plus_b(freeze_origin_and_l, W_l, b_l, name="scores_l")
