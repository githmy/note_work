...
from utils.log_tool import basic_path, project_path, log_path


# 构建模型
class TextCNN(object):
    def __init__(self, server_conf, model_json):
        ...
        self.model_dir = os.path.join(project_path, "models", self.model_name)
        if not os.path.exists(self.model_dir):
            os.makedirs(self.model_dir)
        self.log_dir = os.path.join(log_path, self.model_name)
    ...
    def build(self):
        if 1:
            ...
            with tf.name_scope("output"):
                ...
                W_l = tf.get_variable(
                    "W_l",
                    shape=[num_filters_total + self.label_length_r, self.label_length_l],
                    initializer=tf.contrib.layers.xavier_initializer())
                b_l = tf.Variable(tf.constant(0.1, shape=[self.label_length_l]), name="b_l")
                l2_loss_l += tf.nn.l2_loss(W_l)
                l2_loss_l += tf.nn.l2_loss(b_l)
                self.scores_l = tf.nn.xw_plus_b(origin_and_l, W_l, b_l, name="scores_l")
    ...
    def load_mode(self, modelname):
        # headname = os.path.join(self.model_dir, modelname)
    ...
    def _get_standard_data(self):
        ...
        if os.getenv('prtest') is None:
            ...
            train_file = os.path.join(project_path, "train_compare_origin.csv")
            test_file = os.path.join(project_path, "train_compare_label.csv")
            dict_file = os.path.join(project_path, "review_obj.csv")
    ...
    def get_weigh(self):
        ...
        if os.environ['prtest'] is None:
            ...
            review_json = json.load(open(os.path.join(conf_path, "review_weigh.json"), encoding="utf8"))
            main_json = json.load(open(os.path.join(conf_path, "main_weigh.json"), encoding="utf8"))
            level_json = json.load(open(os.path.join(conf_path, "level_weigh.json"), encoding="utf8"))
