...
class Sequence_chara():
    ...
    def get_label_avelist(self, lists):
        if lists is not None:
            for i1 in lists:
                lup = (pd.rolling_max(self.pf['high'], i1) - self.pf['close'].shift(i1)) / self.pf['close'].shift(i1)
                ldw = (pd.rolling_min(self.pf['low'], i1) - self.pf['close'].shift(i1)) / self.pf['close'].shift(i1)
    ...
    def chara_EVM(self, ndays=5):
        ...
        EVM = dm / br
        EVM_MA = pd.Series(pd.rolling_mean(EVM, ndays), name='chara_' + str(ndays) + '_EVM')
    ...
    def charaR_EVM(self, ndays=5):
        ...
        EVM = dm * br
        EVM_MA = pd.Series(pd.rolling_mean(EVM, ndays), name='charaR_' + str(ndays) + '_EVM')
        self.pf = self.pf.join(EVM_MA)
    ...
    def chara_SMA(self, ndays=5):
        ...
        if ndays is None:
            raise Exception("error: no ndays")
        SMA = pd.Series(pd.rolling_mean(self.pf['close'], ndays), name='chara_' + str(ndays) + '_SMA')
    ...
    def charaR_SMA(self, ndays=5):
        ...
        if ndays is None:
            raise Exception("error: no ndays")
        SMA = pd.Series(pd.rolling_mean(self.pf['close'], ndays) / self.pf['close'].shift(ndays),
    ...
    def charaR_EWMA(self, ndays=5):
        ...
        if ndays is None:
            raise Exception("error: no ndays")
        EMA = pd.Series(pd.ewma(self.pf['close'], span=ndays, min_periods=ndays - 1) / self.pf['close'].shift(ndays),
    ...
    def chara_BB(self, ndays=5):
        ...
        if ndays is None:
            raise Exception("error: no ndays")
        MA = pd.Series(pd.rolling_mean(self.pf['close'], ndays))
        SD = pd.Series(pd.rolling_std(self.pf['close'], ndays))
        b1 = MA + (2 * SD)
    ...
    def charaR_FI(self, ndays=5):
        ...
        FI = pd.Series(self.pf['close'].diff(ndays) / self.pf['close'].shift(ndays) * np.exp(
            pd.rolling_sum(self.pf['charaR_1_1_volrlog'], ndays)), name='charaR_' + str(ndays) + '_FI')
        self.pf = self.pf.join(FI)
