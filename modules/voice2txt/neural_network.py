...
label_file = '/home/mla/data/corpus/data_thchs30/wav_label.txt'
model_path = '/home/mla/data/projectl/models/speech.model'
model_ppath = '/home/mla/data/projectl/models/'
log_dir = '/home/mla/data/projectl/logs/'
...
def get_next_batches(batch_size, wav_max_len, labels_vector, label_max_len):
    global pointer
    batches_wavs = []
    batches_labels = []
...
def speech_to_text_network(X, Y, words_size, n_dim=128, n_blocks=3):
...
def train_speech_to_text_network(wav_max_len, X, Y, words_size, sequence_len, n_batch, batch_size, labels_vector,
                                 label_max_len):
    logit = speech_to_text_network(X, Y, words_size)
    ...
    with tf.Session() as sess:
        ...
        train_summary = tf.summary.FileWriter(log_dir, sess.graph)
        ...
        for epoch in range(16):
            ...
            for batch in range(n_batch):
                batches_wavs, batches_labels = get_next_batches(batch_size, wav_max_len, labels_vector,
                                                                label_max_len)
                print(epoch, 16, batch, n_batch)
                # print(len(batches_wavs), len(batches_wavs[0]))
                # print(len(batches_labels), len(batches_labels[0]))
                if batch == 247:
                    continue
                # print(batches_wavs)
...
# 语音识别
# 把 batch_size 改为1
def speech_to_text(wav_file, X, sequence_len, words_size):
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, tf.train.latest_checkpoint(model_ppath))
...
def train():
    ...
    train_speech_to_text_network(wav_max_len, X, Y, words_size, sequence_len, n_batch, batch_size, labels_vector,
                                 label_max_len)
...
def predict():
    ...
    word_num_map = dict(zip(words, range(len(words))))
    anti_word_num_map = dict(zip(range(len(words)), words))
    ...
    wav_file = "./D32_999.wav"
    print(wav_file)
    res = speech_to_text(wav_file, X, sequence_len, words_size)
    print("".join([anti_word_num_map.get(i1) for i1 in res[0]]))
    print("".join([anti_word_num_map.get(i1 - 1) for i1 in res[0]]))
