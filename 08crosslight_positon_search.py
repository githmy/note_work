...
class ParaSearch_bak(object):
    ...
    def load_history(self):
        ...
        if os.path.isfile(loadfile):
            ...
            # 1. 数据加载
            pdobj = pd.read_csv(loadfile, header=0, encoding="utf8")
            print("contents:", pdobj)
            self.result_json = json.loads(pdobj.to_json(orient='records', force_ascii=False), encoding="utf8")
    ...
    def save_result(self, idn, usetime, status_sig, theta_dis, dy):
        ...
        pdobj.to_csv(loadfile, index=False, header=True, encoding='utf8')
...
class ProcessCtl(object):
    def __init__(self, fit_func, iter_json, exe_json, project_path, run_info_list):
        ...
        self.min_step = self.iter_json["min_step"]
        self.max_process = self.exe_json["max_process"]
        self.para_table = os.path.join(self.project_path, self.exe_json["table"]["para_table"])
    ...
    def load_history(self):
        # todo: 默认数据记录文件
        self.next_cluster_id = 0
        if os.path.isfile(self.para_table):
            print("loadlog: {}".format(self.para_table))
            # 1. 数据加载
            pdobj = pd.read_csv(self.para_table, header=0, encoding="utf8")
        else:
            pass
        if 1:
            # 不存在主动获取数据写入
            self.result_json = json.loads(pdobj.to_json(orient='records', force_ascii=False), encoding="utf8")
            print(type(self.result_json), self.result_json)
...
def main():
    ...
    exe_json = {"apsys": "", "csuprem": "", "pics3d": "", "max_process": 2,
                "table": {"para_table": 'template.csv'}
                }
    project_path = os.path.join("c:\\", "project", "simuproject", "simon", "test_qyb")
    run_info_list = [
        ["apsys", "t.sol", ["@doe_fa", None], ["@doe_fb", None]],
        ["apsys", "t.plt", ["@doe_fa", None], ["@doe_fc", None]]
    ]
    psins = ProcessCtl(fit_func, iter_json, exe_json, project_path, run_info_list)
    psins.call()
    psins.show_result()
