...
def csv2json_noentity(csvfin):
    arrayobjs = {}
    with open(csvfin, "rU") as csvfile:
        res = csv.reader(csvfile)
        for i in res:
            if len(i) > 1 and i[0] != "text":
                if not arrayobjs.has_key(i[1]):
                    arrayobjs.__setitem__(i[1], {'rasa_nlu_data': {'common_examples': []}})
                arrayobjs[i[1]]['rasa_nlu_data']['common_examples'].append({
                    "text": i[0],
...
if __name__ == '__main__':
    rootjson = os.path.join(TXTPATH, "rasa_zh_movie.json")
    logjson = os.path.join(TXTPATH, "logtmpin.json")
    csvout = os.path.join(TXTPATH, "logout.csv")
    ...
    listss = [
        # "rasa_zh_movie.json",
        # "rasa_zh_history_today.json",
        # "rasa_zh_express.json",
        # "rasa_zh_music.json",
        # "rasa_zh_violation_lookup.json",
        # "rasa_zh_navigation.json",
        # "rasa_zh_new_schedule.json",
        "rasa_zh_exchange_rate.json",
        # "rasa_zh_game.json",
        # "rasa_zh_cookbook.json",
        # "rasa_zh_order_food.json",
        # "rasa_zh_horoscope.json",
        # "rasa_zh_chinese_zodiac.json",
        # "rasa_zh_joke.json",
    ]
    for i in listss:
        tmpjson = os.path.join(TXTPATH, i)
        json2csv(tmpjson, csvout)
        # 识别csv错误标签后 生产 主子新json，手动加入数据
        csv2json_noentity(csvout)
        # # 手动去重整合到训练json的数据。
        # log2csv(logjson, csvout)
